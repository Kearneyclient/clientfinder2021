...
def basic():
    ...
    last_date=df_dates['dates'][1]

    new_date=new_date[1:-1]
    from_date=from_date[1:-1]
    last_date=last_date[1:-1]

    # workbook = load_workbook(r"https://docs.google.com/spreadsheets/d/e/2PACX-1vQjKobrW3PCsWIoQwQRJptG9KIJ7z8D2YEgYYTn8v5VUJrxyRBh3KmNI350kcYm5g/pub?output=xlsx")
    # sheet3 = workbook['dates']
    # sheet3["B2"] = f'"{fromdate.strftime("%Y-%m-%d")}"'
    # sheet3["B3"] = df_dates['dates'][2]
    # sheet3["B4"] = f'"{today.strftime("%Y-%m-%d")}"'
    # workbook.save(filename="try_error2.xlsx")
    ...
    df = pd.read_excel('https://docs.google.com/spreadsheets/d/e/2PACX-1vQjKobrW3PCsWIoQwQRJptG9KIJ7z8D2YEgYYTn8v5VUJrxyRBh3KmNI350kcYm5g/pub?output=xlsx')
    df=df.fillna(' ')
    ind_name_list=df['Industry Name'].tolist()
    ind_code_list=df["Industry code"].tolist()
    pm_list=df['Product Margin'].tolist()
        ...
        try:
            ...
            for i in tqdm(iterator):
                ...
                if comparison[date_list[0]]==comparison[date_list[1]]:
                    ...
                    df1=df
                    df=df.head(5).sort_values('N_docs', ascending=False)
                    df=df.reset_index()
                    #print(querytext)
            ...
            leni=len(input_list)
            for h in tqdm(input_list):

                i=1
                #print(f'\nStep {i} / {leni}')
                i+=1
                #df_email=pd.DataFrame
                for list_sel_comp in tqdm(h):
            ...
            for i in range(len(df_final["Monthly Score"])):
                min_list = [df_final["Monthly Score"][i],df_final["Quarterly Score"][i],df_final["Yearly Score"][i]]
                if df_final["Yearly Score"][i] == max(min_list): 
                    df_final["Urgent Need Reqd"][i] = 0
                elif df_final["Quarterly Score"][i] == max(min_list): 
    ...
    #                df_final["Urgent Need Reqd"][i] = 0
                ...
                for j in range(len(df_final)):
                    if i!=j:
                        if df_final['Problem'][i]==df_final['Problem'][j]:
                            if ((type(df_final['Revenue'][j])==float or type(df_final['Revenue'][j])==int)and(type(df_final['Revenue'][i])==float or type(df_final['Revenue'][i])==int)) :
                                if df_final['Revenue'][j]*0.4<df_final['Revenue'][i] or df_final['Revenue'][i]<df_final['Revenue'][j]*2.5:
            ...
            df_final['Monthly Score'] = df_final['Monthly Score'].apply(lambda a: round(a,3))
            df_final['Quarterly Score'] = df_final['Quarterly Score'].apply(lambda a: round(a,3))
            df_final['Yearly Score'] = df_final['Yearly Score'].apply(lambda a: round(a,3))
            df_final['Urgent Need Reqd'] = df_final['Urgent Need Reqd'].apply(lambda a: round(a,3))
            break
        except:
            time.sleep(5)
            print('Starting over')
    ...
    df_list=[]
    for i in range(len(df_final[df_final['Urgent Need Reqd']==1])):
        if (re.split(' ',df_final[df_final['Urgent Need Reqd']==1].iloc[i]['Peer Tickers'][1:])[0]!=''):
            list1=re.split(' ',df_final[df_final['Urgent Need Reqd']==1].iloc[i]['Peer Tickers'][1:])
            df_1=pd.DataFrame(columns=['Company','Ticker','Problem','Revenue','Peer Tickers','Total Score','Monthly Score','Quarterly Score','Yearly Score','Urgent Need Reqd'])
            df_1=df_1.append(df_final[df_final['Urgent Need Reqd']==1].iloc[i],ignore_index=True)
            for j in list1:
                a=df_final[df_final['Ticker']==j].iloc[0]
                df_temp=pd.DataFrame(a).transpose()
                df_1=df_1.append(df_temp,ignore_index=True)
        else:
            continue
                
        df_1.rename(columns = {'Urgent Need Reqd':'Urgency [0-1]'}, inplace=True)
        df_1.drop(['Ticker','Peer Tickers','Total Score'], axis=1, inplace=True)
        



        for i in range(len(df_1['Revenue'])):
            if df_1['Revenue'].iloc[i]==None:
                df_1['Revenue'].iloc[i]='No Accurate Data'
            else:
                if 99999<df_1['Revenue'].iloc[i]<=499999999:
                    df_1['Revenue'].iloc[i]=str(round(df_1['Revenue'].iloc[i]/1000000))+'M$'
                elif 499999999<df_1['Revenue'].iloc[i]:
                    df_1['Revenue'].iloc[i]=str(round(df_1['Revenue'].iloc[i]/1000000000,2))+'B$'
                else:
                    df_1['Revenue'].iloc[i]='No Accurate Data'
    ...
    document.add_paragraph("This reports the companies that have been highlighted through AlphaSense as they could benefit from a project with Kearney PERLab")

    document.add_heading('High Priority Companies', 1)

    for i in range(len(df_list)):
        
        document.add_heading(df_list[i].iloc[0][0],1)
        document.add_paragraph(df_list[i].iloc[0][0] + 'has been highlighted due to a potential "' + df_list[i].iloc[0][1] + f'" issue and having maximum urgency score.')
        t = document.add_table(df_list[i].shape[0]+1, df_list[i].shape[1])
        t.style = 'Table Grid'
        # add the header rows.
        for j in range(df_list[i].shape[1]):
            t.cell(0,j).text = df_list[i].columns[j]

        # add the rest of the data frame
        for k in range(df_list[i].shape[0]):
            for j in range(df_list[i].shape[1]):
                t.cell(k+1,j).text = str(df_list[i].values[k,j])
    df_list=[]
    df_1=''
    df_temp=''
    for i in range(len(df_final[df_final['Urgent Need Reqd']==0.5])):
        if (re.split(' ',df_final[df_final['Urgent Need Reqd']==0.5].iloc[i]['Peer Tickers'][1:])[0]!=''):
            list1=re.split(' ',df_final[df_final['Urgent Need Reqd']==0.5].iloc[i]['Peer Tickers'][1:])
            df_1=pd.DataFrame(columns=['Company','Ticker','Problem','Revenue','Peer Tickers','Total Score','Monthly Score','Quarterly Score','Yearly Score','Urgent Need Reqd'])
            df_1=df_1.append(df_final[df_final['Urgent Need Reqd']==0.5].iloc[i],ignore_index=True)
            for j in list1:
                a=df_final[df_final['Ticker']==j].iloc[0]
                df_temp=pd.DataFrame(a).transpose()
                df_1=df_1.append(df_temp,ignore_index=True)
        ...
        df_list.append(df_1)
    document.add_heading('Medium Priority Companies', 1)

    for i in range(len(df_list)):
        
        document.add_heading(df_list[i].iloc[0][0],1)
        document.add_paragraph(df_list[i].iloc[0][0] + 'has been highlighted due to a potential "' + df_list[i].iloc[0][1] + f'" issue and having moderate urgency score.')
        t = document.add_table(df_list[i].shape[0]+1, df_list[i].shape[1])
        t.style = 'Table Grid'
        # add the header rows.
        for j in range(df_list[i].shape[1]):
    ...
    df_1=''
    df_temp=''
    for i in range(len(df_final[df_final['Urgent Need Reqd']==0])):
        if (re.split(' ',df_final[df_final['Urgent Need Reqd']==0].iloc[i]['Peer Tickers'][1:])[0]!=''):
            list1=re.split(' ',df_final[df_final['Urgent Need Reqd']==0].iloc[i]['Peer Tickers'][1:])
            df_1=pd.DataFrame(columns=['Company','Ticker','Problem','Revenue','Peer Tickers','Total Score','Monthly Score','Quarterly Score','Yearly Score','Urgent Need Reqd'])
            df_1=df_1.append(df_final[df_final['Urgent Need Reqd']==0].iloc[i],ignore_index=True)
            for j in list1:
                a=df_final[df_final['Ticker']==j].iloc[0]
                df_temp=pd.DataFrame(a).transpose()
                df_1=df_1.append(df_temp,ignore_index=True)
        ...
        for i in range(len(df_1['Revenue'])):
            if df_1['Revenue'].iloc[i]==None:
                ...
                if 99999<df_1['Revenue'].iloc[i]<=499999999:
                    df_1['Revenue'].iloc[i]=str(round(df_1['Revenue'].iloc[i]/1000000))+'M$'
                elif 4999999999<df_1['Revenue'].iloc[i]:
                    df_1['Revenue'].iloc[i]=str(round(df_1['Revenue'].iloc[i]/1000000000,2))+'B$'
                else:
                    df_1['Revenue'].iloc[i]='No Accurate Data'
                
        df_list.append(df_1)
    document.add_heading('Low Priority Companies', 1)

    for i in range(len(df_list)):
        
        document.add_heading(df_list[i].iloc[0][0],1)
        document.add_paragraph(df_list[i].iloc[0][0] + 'has been highlighted due to a potential ""' + df_list[i].iloc[0][1] + '" issue and having low urgency score.')
        t = document.add_table(df_list[i].shape[0]+1, df_list[i].shape[1])
        t.style = 'Table Grid'
        # add the header rows.
        for j in range(df_list[i].shape[1]):
...
def result():   
    ...
    if request.method == 'POST':
        ...
        filename = "Final_Report.docx"  # In same directory as script
